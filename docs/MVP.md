...
```python
# Phase 9 实际实现（milvus_lite/index/faiss_hnsw.py）
...
```
pymilvus ──gRPC──→ [ milvus_lite/adapter/grpc/ ] ──→ [ 内部引擎 ]
...
├─ 错误码翻译（MilvusLiteError → grpc Status code）
...
- ✅ **gRPC 适配层**（Phase 10）—— pymilvus 客户端无需改代码即可连接 MilvusLite
