...
| 子阶段 | 内容 | 改动文件 | 工作量 |
|---|---|---|---|
| **9.1** | 补齐 pymilvus quickstart 前置 API：`Collection.create_partition / drop_partition / list_partitions / num_entities / describe` + `search(output_fields=...)` + `LiteVecDB.get_collection_stats` | `engine/collection.py`, `db.py`, `search/executor.py` | S |
...
| 9.1.4 | `LiteVecDB.get_collection_stats(name)` | `db.py` + `tests/test_db.py` |
...
**目标**：在 engine 之上构造 gRPC 服务层，让 pymilvus 客户端无需修改代码即可连接 LiteVecDB。"本地版 Milvus" 协议兼容性的最终一公里。
...
| 10.2.1 | `translators/schema.py`：Milvus FieldSchema ↔ LiteVecDB 双向 | + `tests/adapter/test_grpc_translators_schema.py` |
...
| 10.6.1 | `errors.py`：LiteVecDBError → ErrorCode 映射 | + `tests/adapter/test_grpc_error_mapping.py` |
...
- `python -m litevecdb.adapter.grpc --data-dir ./data --port 19530` 起 server
