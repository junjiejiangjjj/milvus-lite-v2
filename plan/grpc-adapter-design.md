...
LiteVecDB Phase 10 在内部 engine 之上构造一个 gRPC 服务层，让 **pymilvus 客户端无需修改代码即可连接 LiteVecDB**。这是项目作为"本地版 Milvus"在协议兼容性上的最终一公里。
...
3. 方案 C 等于"protocol surface = Milvus，functional surface = LiteVecDB"，pymilvus 能 connect + 大部分 quickstart 调用能跑通，少数 RPC 返回明确的"LiteVecDB 不支持 X"
...
放到 `litevecdb/adapter/grpc/proto/`，用 `grpcio-tools` 生成 `_pb2.py` / `_pb2_grpc.py`，**生成结果 commit 到 repo**（不 runtime 生成，避免 build 时依赖 grpcio-tools）。
...
└── adapter/
    └── grpc/
        ...
        │   ├── expr.py                 # Milvus filter expr ↔ LiteVecDB filter
        ...
        ├── errors.py                   # LiteVecDBError → grpc Status mapping
...
|---|---|---|---|
| `MilvusClient(uri="...")` | (TCP connect) | `LiteVecDB(data_dir)` 在 server 启动时持有 | server 模式只服务一个 data_dir |
...
| `describe_collection(name)` | `DescribeCollection` | `db.get_collection(name).describe()` + schema 序列化 | translator 把 LiteVecDB schema 转回 Milvus proto schema |
...
| `list_databases()` | `ListDatabases` | stub 返回 `["default"]` | LiteVecDB 没有 database 多实例 |
...
1. **明确 UNIMPLEMENTED**（推荐默认）：返回 `grpc.StatusCode.UNIMPLEMENTED` + 友好消息
   ...
   - 错误消息格式：`"LiteVecDB does not support X. Reason: <one-line reason>. See https://...”`
...
- LiteVecDB engine 是行式 — `List[Dict[field_name, value]]`
...
```python
# litevecdb/adapter/grpc/translators/records.py
...
def fields_data_to_records(
    ...
    Raises:
        ...
        UnsupportedFieldTypeError: if a FieldData uses a type LiteVecDB
...
LiteVecDB Phase 8 的 filter grammar 就是抄的 Milvus，**绝大多数表达式直接透传**：
...
```python
# litevecdb/adapter/grpc/translators/expr.py
...
def translate_filter_expr(milvus_expr: str) -> str:
    ...
    For LiteVecDB Phase 9/10, the grammar is a near-superset alignment with
    Milvus, so most expressions pass through unchanged. We only intercept
    function calls that LiteVecDB doesn't implement to give a clear error
    ...
    for func in _UNSUPPORTED_FUNCS:
        ...
        if f"{func}(" in milvus_expr:
            raise FilterUnsupportedError(
                f"Milvus function {func}() is not supported by LiteVecDB. "
...
```python
# litevecdb/adapter/grpc/servicer.py
...
from .translators.schema import milvus_to_litevecdb_schema, litevecdb_to_milvus_schema
...
class MilvusServicer(milvus_pb2_grpc.MilvusServiceServicer):
    def __init__(self, db: LiteVecDB):
        self._db = db
    ...
    def CreateCollection(self, request, context):
        try:
            schema = milvus_to_litevecdb_schema(request.schema)
...
```python
# litevecdb/adapter/grpc/errors.py
...
```python
# litevecdb/adapter/grpc/server.py
...
def run_server(
    ...
    server.start()
    print(f"LiteVecDB gRPC server listening on {addr} (data_dir={data_dir})")
    try:
...
```python
# litevecdb/adapter/grpc/cli.py / __main__.py

import argparse
from .server import run_server

def main():
    parser = argparse.ArgumentParser(prog="python -m litevecdb.adapter.grpc")
...
| 子阶段 | 内容 | 完成标志 | 工作量 |
|---|---|---|---|
| **10.1** | proto 拉取 + stub 生成 + 空 servicer + `run_server` + CLI | `python -m litevecdb.adapter.grpc --data-dir /tmp/x --port 19530` 起 server，pymilvus.connect() 不报错；所有 RPC 返回 UNIMPLEMENTED | M |
...
| **10.6** | 错误码映射 + 异常 wrapping 中间件 + UNIMPLEMENTED 友好消息 | 每种 LiteVecDBError 都有对应的 grpc status code 测试 | S |
...
| `tests/adapter/test_grpc_translators_schema.py` | Milvus FieldSchema ↔ LiteVecDB FieldSchema 双向 |
...
def grpc_server(tmp_path):
    """Start LiteVecDB gRPC server in a thread, yield port, stop after test."""
    from litevecdb.adapter.grpc.server import run_server_in_thread
...
grpc = ["grpcio>=1.50", "protobuf>=4.21"]
all = ["litevecdb[faiss,grpc]"]

[project.scripts]
litevecdb-grpc = "litevecdb.adapter.grpc.cli:main"
```
...
- `python -m litevecdb.adapter.grpc --data-dir ./data --port 19530` 能起 server
